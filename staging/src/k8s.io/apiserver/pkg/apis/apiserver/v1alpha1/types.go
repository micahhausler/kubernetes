...
// verification key and an identity. Exactly one of keys or x509 is required.
//
// keys states both halves here in configuration: the key, and the identity it
// authenticates as. x509 states neither, and takes both from a certificate the
// request carries, which this authenticator's trust anchors have to validate.
// The difference is what the server has to hold. With keys it holds every
// public key and every shared secret, and withdrawing one means editing this
// file. With x509 it holds a certificate authority bundle and nothing per
// client, and a certificate's lifetime is the withdrawal window.
type HTTPSignatureAuthenticator struct {
	...
	// keys are the verification keys this authenticator accepts, and the
	// identity each one authenticates as. A request is authenticated when one of
	// its signatures verifies against the key its keyid names.
	//
	// Mutually exclusive with x509.
	// +optional
	// +listType=atomic
	Keys []HTTPSignatureKey `json:"keys,omitempty"`

	// x509 resolves the key and the identity from a certificate the request
	// carries, rather than from this file.
	//
	...
	//
	// Valid only with keys. A certificate carries its own key, so there is
	// nothing to derive.
	// +optional
	...
	//
	// Required with x509 and invalid with keys, which states its identities
	// directly. This is where the cluster decides what a certificate is allowed
	// to mean, which matters because the identity now comes from an assertion
