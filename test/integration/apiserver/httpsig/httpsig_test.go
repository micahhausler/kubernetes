...
// the property the whole design is for, and it can only be observed at this level:
// the request is byte for byte what the client sent, including its signature.
func TestReplayedRequestIsRejected(t *testing.T) {
	keyFile, publicKey := keyPair(t, "alice", "ecdsa-p256-sha256")
	server := startServer(t, fmt.Sprintf(`
apiVersion: apiserver.config.k8s.io/v1alpha1
kind: AuthenticationConfiguration
httpSignature:
...
`, aliceKeyID, indent(publicKey), aliceUser, signerGrp))
	...
	// Capture the signed request on its way out, then send the same bytes twice.
	...
	// The captured request already carries a signature, so it is replayed through
	// a transport that does not sign, exactly as an attacker would resend it.
	...
	if resp.StatusCode != http.StatusUnauthorized {
		t.Errorf("a replayed request got %d, want 401: the nonce should already have been spent", resp.StatusCode)
	}
}
