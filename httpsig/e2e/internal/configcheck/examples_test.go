...
func TestExamplesAreAccepted(t *testing.T) {
	...
	for _, path := range entries {
		t.Run(filepath.Base(path), func(t *testing.T) {
			...
			if keys == 0 || certs == 0 {
				t.Errorf("got %d key authenticators and %d certificate authenticators, want at least one of each: "+
					"the example exists to show both ways of resolving a signature", keys, certs)
			}
		})
	}
}
